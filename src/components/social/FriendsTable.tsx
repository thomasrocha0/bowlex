...
export type FriendsTableMode = "friends" | "pendingIncoming" | "pendingOutgoing" | "blocked" | "search";
...
export function FriendsTable({ rows, mode, onSelectRow }: FriendsTableProps) {
  return (
    <ScrollView contentContainerStyle={styles.content}>
      {rows.length === 0 && <Text style={styles.empty}>{EMPTY_MESSAGES[mode]}</Text>}
      {rows.map((row) => (
        <FriendRow key={row.profile.id} profile={row.profile} onPress={() => onSelectRow(row)} />
      ))}
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  empty: { color: friendsTableColors.empty, textAlign: "center", marginTop: 24 },
});
